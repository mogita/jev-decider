...
**Tuning on one narrow schema did not cost general decision ability, it bought some.** Against its own frozen base through the identical harness, the adapter is ahead on every macro column: accuracy 0.646 to 0.665, ECE 0.329 to 0.214 untuned, Brier 0.671 to 0.524. The largest single move is `sms_spam`, 0.588 to 0.835, on a source it never saw. The one real regression is `stsb`, 0.416 to 0.304, six ordered levels of semantic similarity being the furthest thing here from picking a category.

Read the raw ECE and the fitted ECE as answering different questions. The published baselines were all fitted on validation splits before being scored, so 0.065 is the comparable figure and it is the lowest in the table; 0.214 is what you get calling this adapter with no calibration step, which is what the section above warns about. Accuracy is identical either way, since temperature scaling cannot move an argmax.

Caveats that cut against these numbers rather than for them: the Qwen3.5 rows are a later base generation, so the clean comparison is against our own base and not against them; this model has one primitive, so every yes-or-no source was asked as a two-way choice, which jev-bench's own probes show is the worse geometry for calibration; `chaosnli` ships no validation split and so enters the fitted macro raw at 0.238; and nothing here says anything about routing to 77 or 151 options, which is where the interesting failures live.
