...
2. **Score the over-long rows both ways.** Thirty-seven of the 111 hard tasks are longer than the 4,000 character input guard, the longest being 14,986. That guard is a serving decision protecting one laptop, not a limit of the model. Counting those rows wrong measures what this project ships and is comparable to the Dohnuts figure, so that is the headline; re-running with the guard raised measures the model and goes in a footnote, marked as not reachable through the public API. One flag, half an hour.
