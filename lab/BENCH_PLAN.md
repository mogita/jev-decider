...
3. **Extend `schema_eval.py`** with BoolQ, XNLI, WikiQA, the UCI SMS spam set, ESCI, ShARC and ContractNLI. All public on the Hub. ContractNLI rows are whole contracts, so feed them whole and chunk only the ones that genuinely exceed the model's context, which is the only limit here that is not a choice. Keep the neutral prompt, since that is the wording the service uses for any schema a caller brings and the score should describe what a caller actually gets. Two hours to add, roughly fifteen minutes per set to run.
